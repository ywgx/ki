...
def find_config():
    cmd = '''find $HOME/.kube -maxdepth 2 -type f -name 'kubeconfig*' 2>/dev/null|egrep '.*' || ( find $HOME/.kube -maxdepth 2 -type f 2>/dev/null|egrep '.*' && grep -l "current-context" `find $HOME/.kube -maxdepth 2 -type f` )'''
